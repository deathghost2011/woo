...
public class CalendarUtil {
	...
	public String[] getChineseDayNames(){
		return chineseDayNames;
	}
	...
	public void setGregorian(int y, int m, int d) {
		...
		gregorianMonth = m;
		gregorianDate = d;
		isGregorianLeap = isGregorianLeapYear(y);//判断是否是闰年
	...
	*/ 
	...
	private static String[] chineseDayNames = {"初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
	...
	//农历部分假日
		...
		public static String getChineseHoliday(int y, int m, int d){
			...
			data = (month < 10 ? "0"+month:String.valueOf(month)) + (day < 10 ? "0"+day:String.valueOf(day));
			for(int i = 0; i < lunarHoliday.length; i++){
				System.out.println(data+"-----------"+lunarHoliday[i].substring(0, 4));
...
//			str = gd + "/" + chineseDayNames[Integer.valueOf(cd) - 1];
			str = chineseDayNames[Integer.valueOf(cd) - 1];
		}
		return str;
	}
	...
	public static int[] getYMD(String date){
		...
		String[] strings = date.split("-");
			dateYMD[0] = Integer.valueOf(strings[0]);
			dateYMD[1] = Integer.valueOf(strings[1]);
			dateYMD[2] = Integer.valueOf(strings[2].split(" ")[0]);
		return dateYMD;
	}
